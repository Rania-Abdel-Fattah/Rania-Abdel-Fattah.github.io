...
var pHeight;
var isMoving=false;
var myProfile,	isHidden=false,AF;
...
var myProfile,myBinder,counter=0;
var numPixels = 20; 
...
if(!document.getElementById("myProfile") ){return;};
...
myProfile.style.width=(pWidth*2)+'px';
myProfile.style.height=pHeight+'px';
myProfile.style.position="relative";
myProfile.style.zIndex="0";

	for(var i=0;i<myProfile.childNodes.length;i++){
		if(myProfile.childNodes[i].nodeType == 1){ //element nodes only
			var original = myProfile.childNodes[i];
			...
			myDiv.style.zIndex=myProfile.childNodes.length-counter;
	...
	if(showBinder){
		...
		myProfile.appendChild(myBinder);
	}
myProfile.style.display=""; //all setup, display the book
}
...
function goright(currLeft,currWidth){
	if(currLeft <= numPixels*2){
		...
		revealDiv.style.left=0+'px';
		revealDiv.style.width=pWidth+'px';
		revealDiv.style.zIndex=revealDiv.style.zIndex-599;
